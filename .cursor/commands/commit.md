...
1. **반드시** 프로젝트의 커밋 스킬을 읽고 그 지침을 그대로 따른다: `.cursor/skills/commit/SKILL.md`
2. 스킬에 적힌 단계(변경 파악, 그룹화, 메시지 초안, 설정 확인, 커밋·푸시 등)를 빠짐없이 수행한다.
3. 스킬의 `config.json`이 있으면(`.cursor/skills/commit/config.json` 또는 `~/.cursor/skills/commit/config.json`) 그 설정을 우선한다.
